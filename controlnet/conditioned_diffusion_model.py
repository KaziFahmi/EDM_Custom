...
from controlnet.model import ControlNet

class ConditionedDiffusion(pl.LightningModule):
    ...
    def __init__(self, cfg, dataset_infos, train_smiles, pretrained_EDM):
        super().__init__()
        ...
        self.pretrained_EDM = pretrained_EDM
        
        self.model = ControlNet(model=self.pretrained_EDM, input_dims=self.input_dims)
    ...
    def training_step(self, data, i):
        ...
        pred = self.forward(z_t, extra_data, condition_input)
    ...
    def sample_batch(self, n_nodes: list, number_chain_steps: int = 50, batch_id: int = 0, keep_chain: int = 0,
        ...
        chains = utils.PlaceHolder(X=torch.zeros((number_chain_steps, keep_chain, n_max), dtype=torch.long),
                                   ...
                                   y=None)
        z_t = z_T
        condition_input = self.condition_data # account for condition_data
        ...
        for s_int in reversed(range(0, self.T, 1 if test else self.cfg.general.faster_sampling)):
            ...
            z_s = self.sample_zs_from_zt(z_t=z_t, s_int=s_array, condition_input=condition_input)
    ...
    def forward(self, z_t, extra_data, condition_input):
        ...
        return self.model(model_input, condition_input)
