...
class ControlNetM(nn.Module):
    ...
    def forward(self, Z_t: utils.PlaceHolder, condtion: utils.PlaceHolder):
        ...
        X = (Z_t.X + zero_conved_condition.X)/2 # Calculating X
        E = (Z_t.E + zero_conved_condition.E)/2 # Calculating E
        y = (Z_t.y + zero_conved_condition.y)/2 # Calculating y
        ...
        node_mask = torch.logical_or(Z_t.node_mask, zero_conved_condition.node_mask) # Calculating node_maxk
        trainable_copy_input = utils.PlaceHolder(X=X, E=E, y=y, pos=pos, charges=None, node_mask=node_mask)
        ...
        # Z_t er bodle non_trainable_output
        # zero_conved_condition er bodle zero_conved_output
        X = (non_trainable_output.X + zero_conved_output.X)/2 # Calculating X
        E = (non_trainable_output.E + zero_conved_output.E)/2 # Calculating E
        y = (non_trainable_output.y + zero_conved_output.y)/2 # Calculating y
        ...
        node_mask = torch.logical_or(non_trainable_output.node_mask, zero_conved_output.node_mask) # Calculating node_maxk
        output = utils.PlaceHolder(X=X, E=E, y=y, pos=pos, charges=None, node_mask=node_mask)
